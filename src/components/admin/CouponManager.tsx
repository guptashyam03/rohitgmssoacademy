            ...
            <tbody className="divide-y divide-gray-800">
              ...
              {coupons.map(c => (
                <tr key={c.id} className="hover:bg-gray-800/50 transition">
                  ...
                  <td className="px-5 py-3 text-gray-300">{c.isPercent ? `${c.discount}%` : `Rs. ${c.discount}`}</td>
                  <td className="px-5 py-3 text-gray-400">{c.usedCount} / {c.maxUses ?? 'No limit'}</td>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-4">
              ...
              <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
                <input type="radio" checked={!form.isPercent} onChange={() => f('isPercent', false)} /> Flat Amount (Rs.)
              </label>
            </div>
          </div>
